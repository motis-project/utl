...
template <typename FromIntType, typename ToIntType>
iota_range(FromIntType, ToIntType) -> iota_range<clear_t<FromIntType>, clear_t<ToIntType>>;

template <typename FromIntType, typename ToIntType>
auto iota(FromIntType&& from, ToIntType&& to) {
  return iota_range{std::forward<FromIntType>(from), std::forward<ToIntType>(to)};
}
