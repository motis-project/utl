  ...
  node* make_node(utl::string name) {
    return nodes_
        .emplace_back(utl::make_unique<node>(
            node{next_node_id_++, 0, utl::vector<edge*>{0u}, std::move(name)}))
        .get();
  }
...
TEST_CASE("graph serialization file") {
  {
    ...
    utl::sfile f{"test.bin", "wb"};
    utl::serialize(f, g);
  }  // EOL graph

  auto b = utl::file("test.bin", "r").content();
  auto const g = utl::deserialize<graph>(b.begin(), b.end());
...
TEST_CASE("graph serialization buf") {
  ...
  auto const g = utl::deserialize<graph>(&buf[0], &buf[0] + buf.size());
  auto const visited = bfs(g->nodes_[0].get());
  unsigned i = 0;
  CHECK((*std::next(begin(visited), i++))->name_ == utl::string{"NODE A"});
  CHECK((*std::next(begin(visited), i++))->name_ == utl::string{"NODE B"});
